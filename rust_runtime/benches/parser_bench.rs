...
use rust_runtime::{SliceParser, ToParser, ByteParser, MemoParser};
...
fn byte_bench(c: &mut Criterion) {
    c.bench_function("byteparser_iterate", |b| {
        b.iter(|| black_box(for _ in ToParser::<ByteParser>::into_parser(INPUT) {}))
    });
}

fn memo_bench(c: &mut Criterion) {
    c.bench_function("memoparser_iterate", |b| {
        b.iter(|| black_box(for _ in ToParser::<MemoParser>::into_parser(INPUT) {}))
    });
}

fn slice_bench(c: &mut Criterion) {
    c.bench_function("sliceparser_iterate", |b| {
        b.iter(|| black_box(for _ in ToParser::<SliceParser>::into_parser(INPUT) {}))
    });
}
